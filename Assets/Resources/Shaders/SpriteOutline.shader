Shader "Sprites/Outline"
{
    Properties
    {
		...
		[KeywordEnum(Inside over sprite, Inside under sprite, Outside)] _OutlinePosition("Outline Position (Frame Only)", Float) = 0
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				float2 offsets;
				if(_OutlinePosition != 2 || _OutlineEnabled == 0) // not outside => all insides
				{
					offsets = float2(0, 0);
				}
				else
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				if(_OutlineEnabled != 0)
				{
					...
					else if(_OutlineMode == 1) // Gradient
					{
						...
						{
							if((_OutlinePosition != 2 && c.a != 0 && 
								(IN.texcoord.y + thicknessY > 1 || SampleSpriteTexture(IN.texcoord + fixed2(0, thicknessY)).a == 0 ||
								IN.texcoord.y - thicknessY < 0 || SampleSpriteTexture(IN.texcoord - fixed2(0, thicknessY)).a == 0 ||
								IN.texcoord.x + thicknessX > 1 || SampleSpriteTexture(IN.texcoord + fixed2(thicknessX, 0)).a == 0 ||
								IN.texcoord.x - thicknessX < 0 || SampleSpriteTexture(IN.texcoord - fixed2(thicknessX, 0)).a == 0)) ||

							(_OutlinePosition == 2 && c.a == 0 &&  
							(SampleSpriteTexture(IN.texcoord + fixed2(0, thicknessY)).a != 0 ||
							SampleSpriteTexture(IN.texcoord - fixed2(0, thicknessY)).a != 0 ||
							SampleSpriteTexture(IN.texcoord + fixed2(thicknessX, 0)).a != 0 ||
							SampleSpriteTexture(IN.texcoord - fixed2(thicknessX, 0)).a != 0)))
							{
								if(_Angle >= 360)
								{
								...
								if(_ConnectedAlpha != 0)
								{
									_GradientOutline1.a *= c.a;
									_GradientOutline2.a *= c.a;
									//outlineC.rgb *= outlineC.a;
								}
					...
					else if(_OutlineShape == 0 && _Thickness > 0) // Contour
					{
						...
						else if(_OutlinePosition == 2 && c.a == 0 &&  
							(SampleSpriteTexture(IN.texcoord + fixed2(0, thicknessY)).a != 0 ||
							SampleSpriteTexture(IN.texcoord - fixed2(0, thicknessY)).a != 0 ||
							SampleSpriteTexture(IN.texcoord + fixed2(thicknessX, 0)).a != 0 ||
							SampleSpriteTexture(IN.texcoord - fixed2(thicknessX, 0)).a != 0))
